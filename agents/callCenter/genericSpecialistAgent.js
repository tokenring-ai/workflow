...
async function process(input, workflowContext, registry, agentConfig = {}) {
  ...
  chatService.systemLine(`[GenericSpecialistAgent:${specialistType}] Starting response...`);
  ...
  const client = await modelRegistry.getFirstOnlineClient({ tags: ['support', specialistType] });
  ...
  try {
    ...
    chatService.systemLine(`[GenericSpecialistAgent:${specialistType}] Generated response of length ${responseText.length}.`);
    return { response: responseText };

  } catch (error) {
    chatService.errorLine(`[GenericSpecialistAgent:${specialistType}] Error during text generation: ${error.message}`);
    console.error(error);
    throw error;
  }
}
