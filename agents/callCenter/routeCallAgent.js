...
async function process(input, workflowContext, registry, agentConfig = {}) {
  ...
  if (!input || typeof input.customerQuery !== 'string') {
    throw new Error('Input must be an object with a "customerQuery" string property.');
  }

  const client = await modelRegistry.getFirstOnlineClient({ tags: ['routing', 'classification'] });
  ...
  try {
    ...
    chatService.systemLine(`[RouteCallAgent] Routing decision: ${JSON.stringify(decision)}`);
    return decision; // This object will be used, the key 'agent_type' contains the routeKey

  } catch (error) {
    chatService.errorLine(`[RouteCallAgent] Error during routing: ${error.message}`);
    console.error(error);
    throw error;
  }
}
