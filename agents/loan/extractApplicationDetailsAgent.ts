...
const applicationDetailsSchema = z.object({
  ...
  loan_amount: z
    .number()
    .describe("The amount of money the client is requesting."),
  loan_time_in_months: z
    .number()
    .int()
    .positive()
    .describe("The duration for which the loan is requested, in months."),
  monthly_income: z
    .number()
    .positive()
    .describe("The client's declared monthly income."),
});

type ApplicationDetails = z.infer<typeof applicationDetailsSchema>;
...
const systemPrompt =
  "You are a first point of contact for a loan company. Your job is to turn client conversation into a structured loan application based on the provided details. Extract all necessary fields accurately.";
...
async function process(
  ...
  try {
    ...
    const extractedDetails = generated.object as ApplicationDetails;

    chatService.systemLine(
      `[ExtractApplicationDetailsAgent] Successfully extracted details: ${JSON.stringify(extractedDetails)}`,
    );
    return extractedDetails;
  } catch (error) {
    const err = error as Error;
    chatService.errorLine(
      `[ExtractApplicationDetailsAgent] Error during extraction: ${err.message}`,
    );
    console.error(error);
