...
export class TranslateAgent extends Runnable {
  ...
  constructor(agentConfig, runnableOpts) {
    ...
    if (!this.agentConfig.targetLanguage) {
      throw new Error('TranslateAgent: targetLanguage must be specified in agentConfig.');
    }
  ...
  async invoke(input, workflowContext) {
    if (!input || typeof input.textToTranslate !== 'string') {
      this.error('TranslateAgent: Input must be an object with a "textToTranslate" string property.');
      throw new Error('TranslateAgent: Input must be an object with a "textToTranslate" string property.');
    }

    const { serviceRegistry } = workflowContext || {};
    if (!serviceRegistry) {
      this.error('TranslateAgent: serviceRegistry not found in workflowContext.');
      throw new Error('TranslateAgent: serviceRegistry not found in workflowContext.');
    }

    const chatService = serviceRegistry.requireFirstServiceByType(ChatService);
    const modelRegistry = serviceRegistry.requireFirstServiceByType(ModelRegistry);

    const { targetLanguage, sourceLanguage } = this.agentConfig;

    // Using this.log for logging via Runnable's event system
    this.log(`Starting translation of text from ${sourceLanguage} to ${targetLanguage}. Text: "${input.textToTranslate.substring(0, 50)}..."`);
    ...
    const client = await modelRegistry.getFirstOnlineClient({ tags: ['translation', `to-${targetLanguage.toLowerCase()}`] });
    ...
    try {
      ...
      const generateTextOptions = workflowContext?.serviceRegistry ? workflowContext.serviceRegistry : undefined;
      ...
      this.error(`Error during translation to ${targetLanguage}: ${err.message}`, err);
