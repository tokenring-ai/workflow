...
const taskSchema = z.object({
  role: z.enum(['developer', 'qa', 'devops']).describe("The role responsible for this task."),
  taskId: z.string().describe("A unique identifier for the task (e.g., 'dev-01', 'qa-01')."),
  taskDescription: z.string().describe("A concise description of the task."),
});

const techLeadOutputSchema = z.array(taskSchema);

const systemPrompt = "You are a tech lead. You receive a feature request from the PM. Your job is to break this down into high-level tasks for a Developer, a QA specialist, and a DevOps specialist to estimate. For each role, define a concise `taskDescription` and a unique `taskId` (e.g., 'dev-01', 'qa-01'). Ensure you provide tasks for all three roles if applicable to the feature.";
...
async function process(input, workflowContext, registry, agentConfig = {}) {
  ...
  if (!input || typeof input.featureRequest !== 'string') {
    throw new Error('Input must be an object with a "featureRequest" string property.');
  }

  const client = await modelRegistry.getFirstOnlineClient({ tags: ['planning', 'decomposition'] });

  try {
    const messages = [{ role: 'user', content: `Feature Request: ${input.featureRequest}` }];
    ...
    chatService.systemLine(`[TechLeadAgent] Successfully broke down feature into ${tasks.length} tasks.`);
    return tasks;

  } catch (error) {
    chatService.errorLine(`[TechLeadAgent] Error during task breakdown: ${error.message}`);
    console.error(error);
    throw error;
  }
}
