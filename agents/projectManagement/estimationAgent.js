...
const estimationSchema = z.object({
  man_days_estimate: z.number().positive().describe("The estimated effort in man-days (e.g., 0.5, 1, 2.5)."),
  reasoning: z.string().describe("A brief reasoning for the provided estimate."),
});
...
async function process(input, workflowContext, registry, agentConfig = {}) {
  ...
  chatService.systemLine(`[EstimationAgent:${role}] Starting estimation for task ID "${input.taskId}"...`);

  if (!input || typeof input.taskDescription !== 'string' || typeof input.role !== 'string' || typeof input.taskId !== 'string') {
    throw new Error('Input must be a task object with role, taskId, and taskDescription string properties.');
  }
  ...
  const systemPrompt = agentConfig.baseSystemPrompt ||
  ...
  const client = await modelRegistry.getFirstOnlineClient({ tags: ['estimation', role] });

  try {
    let userMessageContent = `Please estimate the following task for your role ('${input.role}'):\nTask ID: ${input.taskId}\nDescription: ${input.taskDescription}`;
    if (input.additionalData) { // If TechLeadAgent provided more context
    ...
    chatService.systemLine(`[EstimationAgent:${role}] Successfully estimated task ID "${input.taskId}": ${JSON.stringify(estimation)}`);
    return estimation;

  } catch (error) {
    chatService.errorLine(`[EstimationAgent:${role}] Error during estimation for task ID "${input.taskId}": ${error.message}`);
    console.error(error);
    throw error;
  }
}
