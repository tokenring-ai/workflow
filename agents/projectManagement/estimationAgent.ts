...
type EstimationResult = z.infer<typeof estimationSchema>;
...
async function process(
  ...
  try {
    ...
    const estimation = generated.object as EstimationResult;
    ...
    chatService.errorLine(
      `[EstimationAgent:${role}] Error during estimation for task ID "${input.taskId}": ${err.message}`,
    );
    console.error(error);
    throw error;
  }
