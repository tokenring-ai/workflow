...
const schema = {
 ...
 required: ["requiresFiles", "searchFilesFor", "searchWebFor", "taskDescription"]
};

const systemPrompt = `You are a prompt analyzer, analyzing a prompt that will be fed to an AI model. Given the following chat payload, generate:
 ...
 * @param {string} params.originalPrompt - The initial user prompt.
 * @param {TokenRingRegistry} registry - The package registry
 */
async function contextDiscovery({ originalPrompt }, registry) {
 const modelRegistry = registry.requireFirstServiceByType(ModelRegistry);
 const chatService = registry.requireFirstServiceByType(ChatService);
 const filesystem = registry.requireFirstServiceByType(FileSystemService);

 const systemPrompt = chatService.getInstructions();
 const request = await createChatRequest({ input: originalPrompt, systemPrompt }, registry);

 let input = request.messages.map(({role, content}) => `"${role}": "${content}"`).join(',\n  ');
...
${input.slice(-5000)}`;
 ...
 const fileIndexMessages = await Promise.all(analysis.searchFilesFor.map(async query =>
  flow(`Search files for ${query}`, async () => {
   const results = await filesystem.grep(query, { });
   if (results.length > 0) {
...
export default class DiscoveryRunnable extends Runnable {
  async *invoke(context, { serviceRegistry }) {
    const request = await contextDiscovery({ originalPrompt: context.prompt }, serviceRegistry);
    context.request = request;
    yield { type: 'log', level: 'info', message: 'Discovery complete', timestamp: Date.now(), runnableName: this.name };
    return context;
  }
}