...
export const description = "Breaks down a natural language prompt into subtasks and then runs each subtask through the analyze-prompt workflow.";

// Step 2: Define Input Schema
export const inputSchema = z.object({
  prompt: z.string().describe("The main user prompt to break down into subtasks"),
});
...
export const outputSchema = z.array(analyzePromptWorkflow.outputSchema).describe("An array of analysis results for each subtask");

// Step 3a: Helper function to break down the main prompt into subtask prompts using an AI model
async function breakDownPromptIntoSubtasks(mainPrompt, registry) {
  ...
  const client = await modelRegistry.getFirstOnlineClient({ tags: ['plan'] });

  const subtaskSchema = z.object({
    subPrompts: z.array(z.string()).describe("An array of sub-prompts derived from the main prompt, designed to be executed sequentially or in parallel if possible."),
  });
...
export async function execute({ prompt }, registry) {
  ...
  chatService.systemLine(`[TaskPlanner] Starting to break down prompt: "${prompt}"`);

  // Step 3a: Break down the prompt into subtasks
  const subtaskPrompts = await flow("Break down main prompt into subtasks", () =>
    breakDownPromptIntoSubtasks(prompt, registry)
  );

  if (!subtaskPrompts || subtaskPrompts.length === 0) {
    chatService.warningLine("[TaskPlanner] No subtasks were generated from the prompt. The prompt might have been too simple or the AI could not determine subtasks.");
    return []; // Return empty array if no subtasks are generated
  }

  chatService.systemLine(`[TaskPlanner] Generated ${subtaskPrompts.length} subtasks:`);
  subtaskPrompts.forEach((subtask, index) => {
    chatService.systemLine(`  ${index + 1}. ${subtask}`);
  });

  const allSubtaskAnalysisResults = [];
  ...
  for (let i = 0; i < subtaskPrompts.length; i++) {
    ...
    chatService.systemLine(`[TaskPlanner] Analyzing subtask ${i + 1}/${subtaskPrompts.length}: "${subtaskPrompt}"`);
    
    try {
      const analysisResult = await flow(`Analyze subtask: ${subtaskPrompt.substring(0, 50)}...`, () =>
        workflowService.run(analyzePromptWorkflow, { prompt: subtaskPrompt }, registry)
      );
      allSubtaskAnalysisResults.push(analysisResult);
      chatService.systemLine(`[TaskPlanner] Finished analyzing subtask ${i + 1}/${subtaskPrompts.length}.`);
    } catch (error) {
      chatService.errorLine(`[TaskPlanner] Error analyzing subtask ${i + 1}/${subtaskPrompts.length}: "${subtaskPrompt}". Error: ${error.message}`);
  ...
  chatService.systemLine(`[TaskPlanner] Finished processing all ${subtaskPrompts.length} subtasks.`);
