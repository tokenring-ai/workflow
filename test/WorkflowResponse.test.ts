...
async function* mockAsyncGenerator<T = any>(
  ...
  returnValue: T = undefined as unknown as T,
  throwError: Error | null = null
): WorkflowResponseType<T> {
...
describe("WorkflowResponse", () => {
  const baseEventProps = (runnableName?: string): BaseWorkflowEvent => ({
    runnableName: runnableName || "TestRunnable",
    workflowInstanceId: "wf-instance-123",
    traceId: "trace-abc",
    timestamp: Date.now(),
    type: ""  // This will be overridden in each specific event
  });
  ...
  describe("stream()", () => {
    ...
    it("should allow multiple iterations by buffering events (replayability)", async () => {
      ...
      const firstPassEvents: WorkflowEvent[] = [];
      for await (const event of response.stream()) {
        firstPassEvents.push(event);
      }

      const secondPassEvents: WorkflowEvent[] = [];
      for await (const event of response.stream()) {
      ...
      expect((response as any)._eventBuffer.length).toBe(3); // Internal check: buffer should be full
    });
    ...
    it("should handle errors from the underlying generator and yield a workflow_error event", async () => {
      ...
      expect((errorEvent as any)?.error?.message).toBe("Generator failed!");
      expect((response as any)._consumptionError).toBe(error);
    });
  });
  ...
  describe("response()", () => {
    ...
    it("should cache the response promise", async () => {
      ...
      expect((response as any)._responsePromise).toBeInstanceOf(Promise);
    });
  ...
  describe("outputSchema()", () => {
    ...
    it("should cache the schema promise", async () => {
      ...
      const schemaPromise1 = (response as any)._schemaPromise;
      await response.outputSchema();
      const schemaPromise2 = (response as any)._schemaPromise;
  ...
  describe("allEvents()", () => {
    ...
    it("should include error event if stream fails", async () => {
      ...
      expect((response as any)._consumptionError).toBe(error);
      const bufferedEvents = (response as any)._eventBuffer; // Access internal buffer for test verification

      expect(
        bufferedEvents.some(
          (e: WorkflowEvent) =>
            e.type === "workflow_error" &&
            (e as any).error?.message === "Failed during allEvents",
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Interactions between methods", () => {
    it("should allow calling response() after partially consuming stream()", async () => {
      ...
      const remainingEvents: WorkflowEvent[] = [];
