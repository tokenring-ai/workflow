...
class SimpleRunnable extends Runnable {
	async *invoke(input: number, context: { abortSignal: AbortSignal }): AsyncGenerator<{ type: string, message: string, timestamp: number }, number, unknown> {
		for (let i = 0; i < 3; i++) {
...
describe("WorkflowService", () => {
	...
	it("can cancel a running workflow", async () => {
		class SlowRunnable extends Runnable {
			async *invoke(_: unknown, context: { abortSignal: AbortSignal }): AsyncGenerator<{ type: string, message: string, timestamp: number }, string, unknown> {
				for (let i = 0; i < 100; i++) {
					if (context.abortSignal.aborted) {
						throw new Error("aborted");
					}
