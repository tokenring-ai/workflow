 ...
 */
async function* mockAsyncGenerator(events, returnValue = undefined, throwError = null) {
...
describe('WorkflowResponse', () => {
  ...
  describe('stream()', () => {
    it('should allow iterating over events from the generator', async () => {
      const events = [
        { ...baseEventProps('Step1'), type: 'step_start', input: { data: 'start' } },
        { ...baseEventProps('Step1'), type: 'final_output', data: { result: 'done' } },
        { ...baseEventProps('Step1'), type: 'step_end', output: { result: 'done' }, durationMs: 10 },
      ];
      // @ts-ignore
      ...
      expect(collectedEvents.map(e => e.type)).toEqual(['step_start', 'final_output', 'step_end']);
    ...
    it('should allow multiple iterations by buffering events (replayability)', async () => {
      ...
      expect(secondPassEvents.map(e => e.type)).toEqual(['step_start', 'final_output', 'step_end']);
    ...
    it('should handle errors from the underlying generator and yield a workflow_error event', async () => {
      ...
      const response = new WorkflowResponse(generator, {runnableName: 'ErrorWorkflow'});
      ...
      const errorEvent = collectedEvents.find(e => e.type === 'workflow_error');
  ...
  describe('response()', () => {
    it('should resolve with data from the final_output event', async () => {
      const events = [
        ...
        { ...baseEventProps('R'), type: 'final_output', data: { final: 'result' } },
        { ...baseEventProps('R'), type: 'step_end', output: { final: 'result' } },
      ];
      // @ts-ignore
    ...
    it('should cache the response promise', async () => {
      const events = [{ ...baseEventProps('C'), type: 'final_output', data: 'cached' }];
      // @ts-ignore
    ...
    it('should reject if the stream errors before a final_output event', async () => {
      ...
      const generator = mockAsyncGenerator(events, undefined, error);
      const response = new WorkflowResponse(generator);
      await expect(response.response()).rejects.toThrow('Stream error before output');
    });

    it('should resolve with generator TReturn if no final_output event is yielded', async () => {
        const events = [
            { ...baseEventProps('TR'), type: 'step_start' },
            { ...baseEventProps('TR'), type: 'step_end', output: { value: 'from return' } }, // output here is from TReturn
        ];
        // @ts-ignore
        ...
        await expect(wfResponse.response()).resolves.toEqual({ value: 'from return' });
    });
  });
  ...
  describe('outputSchema()', () => {
    ...
    it('should resolve undefined if no schema_definition event is found', async () => {
      const events = [{ ...baseEventProps('S'), type: 'final_output', data: 'done' }];
      // @ts-ignore
      const generator = mockAsyncGenerator(events);
      const response = new WorkflowResponse(generator);
    ...
    it('should cache the schema promise', async () => {
      ...
      const events = [{ ...baseEventProps('SC'), type: 'schema_definition', schema }];
      // @ts-ignore
      const generator = mockAsyncGenerator(events);
      const response = new WorkflowResponse(generator);
    ...
    it('should reject if stream errors before schema or if no schema found and stream errored', async () => {
      ...
      const generator = mockAsyncGenerator(events, undefined, error);
      const response = new WorkflowResponse(generator);
      await expect(response.outputSchema()).rejects.toThrow('Schema stream error');
    });
  });
  ...
  describe('allEvents()', () => {
    it('should collect all events from the stream', async () => {
      const events = [
        ...
        { ...baseEventProps('A'), type: 'log', level: 'info', message: 'hello' },
      ...
      expect(collected.map(e => e.type)).toEqual(['step_start', 'log', 'final_output', 'step_end']);
    });
    ...
    it('should include error event if stream fails', async () => {
        ...
        expect(bufferedEvents.some(e => e.type === 'workflow_error' && e.error?.message === 'Failed during allEvents')).toBe(true);
    });
  });
  ...
  describe('Interactions between methods', () => {
    it('should allow calling response() after partially consuming stream()', async () => {
        ...
        for await (const event of stream) { // This will start from where stream left off internally for original generator
        ...
        expect(all.map(e => e.type)).toEqual(['step_start', 'output_chunk', 'final_output', 'step_end']);
    });
  });
});
