...
import WorkflowService from "../WorkflowService.js";
 ...
 export const description = "/analyze <option1=val1 option2=val2 ...> -- <prompt>";
 ...
 export async function execute(remainder, registry) {
  ...
  if (!remainder || !remainder.trim()) {
   chatService.errorLine('Usage: /analyze <option1=val1 option2=val2 ...> -- <prompt>');
   return;
  }
  ...
  const analysisResult = await workFlowService.run(analyzePromptWorkflow, {prompt}, registry);

  if (analysisResult.isExceptionallyComplex) {
   chatService.systemLine('[AnalyzeCommand] Task is exceptionally complex. Running task planner...');
   const subTaskAnalyses = await workFlowService.run(planTasksWorkflow, {prompt}, registry);

   if (!subTaskAnalyses || subTaskAnalyses.length === 0) {
    chatService.warningLine('[AnalyzeCommand] Task planner did not return any subtasks. Nothing to execute.');
    return;
   }

   chatService.systemLine(`[AnalyzeCommand] Task planner generated ${subTaskAnalyses.length} subtasks. Executing them sequentially...`);
   ...
   for (let i = 0; i < subTaskAnalyses.length; i++) {
    ...
    if (subTask.error) {
     chatService.errorLine(`[AnalyzeCommand] Skipping subtask ${i + 1} due to error during its planning/analysis: ${subTask.error}`);
     continue;
    }
    chatService.systemLine(`[AnalyzeCommand] Executing subtask ${i + 1}/${subTaskAnalyses.length}: ${subTask.input.find(m => m.role === 'user')?.content?.substring(0, 100) || 'User prompt not found'}...`);
   ...
   chatService.systemLine('[AnalyzeCommand] Task is not exceptionally complex. Running directly...');
   return runChat({
