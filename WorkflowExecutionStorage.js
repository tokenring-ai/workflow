...
export class EphemeralWorkflowExecutionStorage extends WorkflowExecutionStorage {
  ...
  async createExecution(context) {
    const id = context.id || `exec_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
    this.executions.set(id, { id, context, input: context.input, events: [], status: 'created', output: undefined });
