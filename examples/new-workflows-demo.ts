...
interface MockChatService {
  systemLine: (message: string) => void;
  errorLine: (message: string) => void;
  userLine: (user: string, message: string) => void;
  assistantLine: (assistant: string, message: string) => void;
}
...
interface WorkflowContext {
  ...
  options: {
    ...
    requireApprovalOnCriticalFailure: boolean;
...
interface RoutingWorkflowDef {
  id: string;
  name: string;
  description: string;
  routerAgent: {
    agentModulePath: string;
    outputKeyForRouting: string;
  };
...
interface OrchestratorWorkflowDef {
  id: string;
  name: string;
  description: string;
  orchestratorAgent: {
...
interface EvaluatorOptimizerWorkflowDef {
  id: string;
  name: string;
  description: string;
  generatorAgent: {
...
const baseWorkflowContext: WorkflowContext = {
  sharedData: {},
...
async function demoChainingWorkflow(): Promise<void> {
  console.log("\n--- Running Chaining Workflow Demo (Loan Application) ---");

  const loanChainDef: ChainingWorkflowDef = {
...
async function demoRoutingWorkflow(): Promise<void> {
  console.log("\n--- Running Routing Workflow Demo (Call Center) ---");

  const callCenterRouteDef: RoutingWorkflowDef = {
    id: "callCenterRouter",
...
async function demoSimpleParallelWorkflow(): Promise<void> {
...
async function demoOrchestratorWorkersWorkflow(): Promise<void> {
  console.log(
  ...
  const projectEstimationDef: OrchestratorWorkflowDef = {
...
async function demoEvaluatorOptimizerWorkflow(): Promise<void> {
  console.log(
  ...
  const articleCreationDef: EvaluatorOptimizerWorkflowDef = {
