...
function createEventBase(
  ...
  runnableName: string
): Pick<BaseWorkflowEvent, "runnableName" | "workflowInstanceId" | "traceId" | "timestamp"> {
  return {
    runnableName,
...
const step2 = new RunnableLambda(
  async function* (input: Step1Output, context?: WorkflowContext) {
    ...
    yield {
      ...
      type: "log",
      level: "info",
      message: `Executing. Input: ${JSON.stringify(input)}.`,
    };
    await new Promise((resolve) => setTimeout(resolve, 50));
    ...
    if (input.value === 13) {
      yield {
        ...
        type: "log",
        level: "error",
        message: `SIMULATING FAILURE for input value 13!`,
      };
 ...
 */
export async function runWorkflowServiceDemo(): Promise<void> {
  ...
  const simpleWorkflow = new RunnableSequence([step1, step2, step3], {
    name: "SimplePersistentWorkflow",
  });
  ...
  wfService.on("workflow_started", (event) => {
    console.log(
      `[WFService EVENT] Workflow Started: DefID=${event.definitionId}, InstanceID=${event.workflowInstanceId}, Input: ${JSON.stringify(event.input)}`,
    );
  ...
  async function processWorkflowResponse(
    ...
    console.log(
      `Streaming events for ${caseName} (Instance: ${workflowResponse.workflowInstanceId}):`,
    );
    try {
      for await (const event of workflowResponse.stream()) {
        ...
        if (event.type === "log")
          eventSummary += ` [${event.level}]: ${event.message}`;
        if (event.type === "step_start")
          eventSummary += ` | Input: ${JSON.stringify(event.input)}`;
        ...
        if (event.type === "final_output")
          eventSummary += ` | Data: ${JSON.stringify(event.data)}`;
