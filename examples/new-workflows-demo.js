...
const mockChatService = {
  ...
  assistantLine: (assistant, message) => console.log(`[ASSISTANT: ${assistant}] ${message}`),
};

const mockAiClient = {
  generateObject: async ({ messages, schema, prompt }, registry) => {
    mockChatService.systemLine(`MockAIClient.generateObject called. System Prompt: ${prompt}`);
    if (schema) {
      mockChatService.systemLine(`(MockAIClient notes that an output schema was provided: ${schema.constructor.name === 'ZodObject' ? 'Zod schema detected' : 'Schema present'})`);
    }
    const userInput = messages.find(m => m.role === 'user')?.content || "";

    if (prompt?.includes("loan application")) { // For extractApplicationDetailsAgent
      ...
      if (userInput.match(/I'm (\w+)/i)) name = userInput.match(/I'm (\w+)/i)[1];
      else if (userInput.match(/name is (\w+)/i)) name = userInput.match(/name is (\w+)/i)[1];
      ...
      const loanTimeInMonths = durationMatch ? parseInt(durationMatch[1]) * 12 : 0;
      ...
      return { object: { name, loan_amount: loanAmount, loan_time_in_months: loanTimeInMonths, monthly_income: monthlyIncome } };
    }
    else if (prompt?.includes("decide if a client can be further processed")) { // For loanGateAgent
      const appData = JSON.parse(userInput);
      const isAccepted = (appData.loan_amount / appData.loan_time_in_months) < (0.3 * appData.monthly_income) && appData.loan_amount > 0;
      return { object: { is_client_accepted: isAccepted, denial_reason: isAccepted ? undefined : "Loan installment exceeds 30% of monthly income or invalid amount." } };
    }
    else if (prompt?.includes("redirect the client to a correct agent")) { // For routeCallAgent
      if (userInput.toLowerCase().includes("internet") || userInput.toLowerCase().includes("technical")) {
        return { object: { agent_type: 'technical' } };
      } else if (userInput.toLowerCase().includes("bill") || userInput.toLowerCase().includes("account")) {
        return { object: { agent_type: 'account' } };
      } else if (userInput.toLowerCase().includes("payment") || userInput.toLowerCase().includes("finance")) {
        return { object: { agent_type: 'finance' } };
      }
      return { object: { agent_type: 'unknown' } };
    }
    else if (prompt?.includes("aggregating translations")) { // For aggregateTranslationsAgent
        // Input will be a stringified version of the array of translation outputs + original text
        const originalTextMatch = userInput.match(/Original Text:\n(.*?)\n\nTranslations:\n/s);
        const originalText = originalTextMatch ? originalTextMatch[1] : "Original text not found in mock.";
        ...
        translationBlocks.forEach(block => {
            ...
            if (langMatch) {
                ...
                translations.push({ language: lang, text: text.trim(), notes: "Mock aggregated." });
            }
        });
        return { object: { original_text: originalText, aggregated_translations: translations, summary_notes: "All translations aggregated by mock." } };
    }
    else if (prompt?.includes("break this down into high-level tasks")) { // For TechLeadAgent
        const featureRequest = userInput.replace('Feature Request: ', '');
        return { object: [
            { role: 'developer', taskId: 'dev-001', taskDescription: `Implement UI for ${featureRequest}` },
            { role: 'developer', taskId: 'dev-002', taskDescription: `Implement API for ${featureRequest}` },
            { role: 'qa', taskId: 'qa-001', taskDescription: `Write test cases for ${featureRequest}` },
            { role: 'devops', taskId: 'devops-001', taskDescription: `Setup deployment for ${featureRequest}` },
        ]};
    }
    else if (prompt?.includes("estimate the effort in man-days")) { // For EstimationAgent
        ...
        return { object: { man_days_estimate: estimate, reasoning: `Mock reasoning for estimating ${taskDesc.substring(0,30)}... at ${estimate} days.` } };
    }
    else if (prompt?.includes("Scrum Master") && prompt.includes("summarize these estimations")) { // For ScrumMasterAgent
        const estimationsText = userInput.match(/Individual Estimations from Team:\n([\s\S]*?)\n\nPlease generate/s);
        ...
        if (estimationsText && estimationsText[1]) {
            ...
            estLines.forEach(lineSet => {
                ...
                if (roleMatch && taskIdMatch && taskMatch && estimateMatch && reasoningMatch && statusMatch) {
    ...
    else if (prompt?.includes("writer's supervisor") && prompt.includes("evaluate if it meets the requirements")) { // For ArticleSupervisorAgent
        const articleText = userInput.match(/Article Text:\n---\n([\s\S]*?)\n---/s)?.[1] || "";
        ...
        if (articleWriterCallCount > 1 || articleText.includes("revised based on feedback")) { // articleWriterCallCount is a global mock helper
            satisfied = true;
            feedback = "Much better, this meets the requirements!";
        }
         if (articleText.length < 30 && articleWriterCallCount === 1) { // Example: too short on first try
            feedback = "The article is too short. Please elaborate further, especially on the impact of AI Agent Workflows.";
            satisfied = false;
        } else if (articleText.length < 60 && articleWriterCallCount === 2 && articleText.includes("revised based on feedback")) {
    ...
    mockChatService.errorLine(`MockAIClient.generateObject: Unhandled prompt scenario for schema generation. Prompt: ${prompt}`);
    return { object: { error: "Unhandled prompt in mock generateObject for this agent/prompt." } };
  },
  generateText: async ({ messages, prompt }, registry) => {
    mockChatService.systemLine(`MockAIClient.generateText called. System Prompt: ${prompt}`);
    const userInput = messages.find(m => m.role === 'user')?.content || "";

    if (prompt?.includes("translate the following text")) { // For translateAgent
        if (prompt.includes("to German")) return { text: `GERMAN: ${userInput}` };
        if (prompt.includes("to Spanish")) return { text: `SPANISH: ${userInput}` };
        if (prompt.includes("to Polish")) return { text: `POLISH: ${userInput}` };
        return { text: `TRANSLATION_UNHANDLED_LANG: ${userInput}`};
    }
    else if (prompt?.includes("You are a writer.")) { // For ArticleWriterAgent
        ...
        if (userInput.includes("Previous Article:")) {
            ...
            const feedback = feedbackMatch ? feedbackMatch[1] : "general improvements";
            return { text: `This is a mock revised article about ${topic} considering constraint "${constraint}" (revised based on feedback: ${feedback.substring(0,30)}...). It is now longer and more detailed.` };
        }
        return { text: `This is a mock first draft article about ${topic} fulfilling constraint "${constraint}". It's probably okay.` };
    }

    const specialistTypeMatch = prompt?.match(/You are a (\w+) specialist/i);
    const specialistType = specialistTypeMatch ? specialistTypeMatch[1] : "general";
    return { text: `This is a mocked ${specialistType} support response regarding: "${userInput}"` };
  }
};
...
const mockModelRegistry = {
   getFirstOnlineClient: async (config) => {
    mockChatService.systemLine(`MockModelRegistry.getFirstOnlineClient called with config: ${JSON.stringify(config)}`);
    return mockAiClient;
  }
};
...
const mockRegistry = {
  requireFirstServiceByType: (typeConstructor) => {
    ...
    if (typeConstructor.name === 'ChatService' || typeConstructor.prototype.constructor.name === 'ChatService') {
      return mockChatService;
    }
    if (typeConstructor.name === 'ModelRegistry' || typeConstructor.prototype.constructor.name === 'ModelRegistry') {
      return mockModelRegistry;
    }
    mockChatService.errorLine(`MockRegistry: Service type "${typeConstructor.name}" not found.`);
    return null;
  },
  getService: (serviceName) => { // Added for conceptual HITL service, though it's null in orchestrator
...
const baseWorkflowContext = {
  ...
  options: { // Default options for orchestrators
    breadth: 3,
    maxPasses: 1,
    maxIterations: 2,
...
async function demoChainingWorkflow() {
    ...
    steps: [
      {
        ...
        outputSchema: z.object({
          ...
          loan_time_in_months: z.number().positive().int().describe("Loan duration in months"),
          monthly_income: z.number().positive().describe("Client's gross monthly income")
        }),
  ...
  const initialLoanInput = { conversation: "Hi, my name is Alex. I'd like to apply for a loan of $10000 for 3 years. I make $60000 a year." };
  ...
  const chainResult = await runChainingWorkflow(loanChainDef, initialLoanInput, chainWorkflowContext, mockRegistry);
  ...
  const callCenterRouteDef = {
    ...
    routes: {
      "technical": {
        ...
        config: { specialistType: "Technical", systemPrompt: "You are a friendly Technical Support specialist. Help with the tech issue." }
      },
      "account": {
        id: "accountSupport",
        agentModulePath: "../agents/callCenter/genericSpecialistAgent.js",
        config: { specialistType: "Account", systemPrompt: "You are an Account specialist. Help with account queries." }
      },
      "finance": {
        id: "financeSupport",
        agentModulePath: "../agents/callCenter/genericSpecialistAgent.js",
        config: { specialistType: "Finance", systemPrompt: "You are a Finance specialist. Help with payment issues." }
      }
    },
    defaultRoute: {
      id: "generalSupport",
      agentModulePath: "../agents/callCenter/genericSpecialistAgent.js",
      config: { specialistType: "General", systemPrompt: "You are a General Support agent. Try to help with the query." }
    },
    passRouterOutputToRoutedAgent: true // Merge router's output with initialInput for the specialist
  };

  const initialCallInput1 = { customerQuery: "Hello, my internet connection is very slow today!" };
  const routeWorkflowContext1 = JSON.parse(JSON.stringify(baseWorkflowContext));

  console.log(`\nRouting query: "${initialCallInput1.customerQuery}"`);
  const routeResult1 = await runRoutingWorkflow(callCenterRouteDef, initialCallInput1, routeWorkflowContext1, mockRegistry);
  console.log("Routing Workflow Result 1:");
  console.log(JSON.stringify(routeResult1, null, 2));

  const initialCallInput2 = { customerQuery: "I need to update my billing address." };
  const routeWorkflowContext2 = JSON.parse(JSON.stringify(baseWorkflowContext));

  console.log(`\nRouting query: "${initialCallInput2.customerQuery}"`);
  const routeResult2 = await runRoutingWorkflow(callCenterRouteDef, initialCallInput2, routeWorkflowContext2, mockRegistry);
...
async function main() {
  ...
  if (!mockRegistry.requireFirstServiceByType(function ChatService(){}) ||
      !mockRegistry.requireFirstServiceByType(function ModelRegistry(){}).getFirstOnlineClient) {
...
async function demoSimpleParallelWorkflow() {
  ...
  const parallelWorkflowContext = JSON.parse(JSON.stringify(baseWorkflowContext)); // Fresh context

  const textToTranslate = "AI Agents can work together in workflows. This is a powerful concept.";
  ...
  const translationParallelDef = {
    ...
    description: "Translates a text into multiple languages in parallel and then aggregates the results.",
    workerAgents: [
      {
        id: "translator-de",
...
async function demoOrchestratorWorkersWorkflow() {
  ...
  const projectEstimationDef = {
    ...
    description: "A Tech Lead breaks down a feature, specialists estimate their parts, and a Scrum Master aggregates.",
    orchestratorAgent: {
      agentModulePath: "../agents/projectManagement/techLeadAgent.js",
    },
    workerAgentsMap: {
      "developer": { agentModulePath: "../agents/projectManagement/estimationAgent.js", config: { roleContext: "You are a senior software developer."} },
      "qa": { agentModulePath: "../agents/projectManagement/estimationAgent.js", config: { roleContext: "You are a QA engineer."} },
      "devops": { agentModulePath: "../agents/projectManagement/estimationAgent.js", config: { roleContext: "You are a DevOps engineer."} },
    },
    aggregatorAgent: {
  ...
  const initialFeatureInput = { featureRequest: "Develop a new user profile page with avatar upload and activity feed." };
  ...
  result.orchestratorTasks?.forEach(task => console.log(JSON.stringify(task, null, 2)));
  console.log("\nOrchestrator-Workers Worker Execution Details:");
  result.workerExecutionDetails?.forEach(detail => console.log(JSON.stringify(detail, null, 2)));
...
async function demoEvaluatorOptimizerWorkflow() {
  ...
  const articleCreationDef = {
    ...
    description: "A writer agent drafts an article, a supervisor evaluates, and the writer refines based on feedback.",
    generatorAgent: {
    ...
    evaluatorAgent: {
      ...
      config: { constraint: "The article must be between 3 and 6 sentences and discuss the impact of AI." }
    },
    initialGeneratorInput: {
      topic: "The Impact of AI Agent Workflows",
      constraint: "The article must be between 3 and 6 sentences and discuss the impact of AI."
  ...
  console.log(`Satisfied: ${result.wasSatisfied}, Iterations: ${result.iterationsTaken}`);
