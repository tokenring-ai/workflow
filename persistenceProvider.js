...
export class PersistenceProvider {
   ...
   * @abstract
   */
  async saveWorkflowState(workflowInstanceId, definitionId, nextStepId, lastOutput, currentWorkflowContext) {
    ...
    throw new Error("PersistenceProvider.saveWorkflowState must be implemented by subclasses.");
  }
  ...
  async loadWorkflowState(workflowInstanceId) {
    throw new Error("PersistenceProvider.loadWorkflowState must be implemented by subclasses.");
  }
  ...
  async clearWorkflowState(workflowInstanceId) {
    throw new Error("PersistenceProvider.clearWorkflowState must be implemented by subclasses.");
  }
}
...
export class InMemoryPersistenceProvider extends PersistenceProvider {
  ...
  async saveWorkflowState(workflowInstanceId, definitionId, nextStepId, lastOutput, currentWorkflowContext) {
    ...
    console.log(`[InMemoryPersistence] State saved for ${workflowInstanceId}. Next step: ${nextStepId}`);
  ...
  async loadWorkflowState(workflowInstanceId) {
    ...
    if (stateRecord) {
      console.log(`[InMemoryPersistence] State loaded for ${workflowInstanceId}. Current step: ${stateRecord.currentStepId}`);
      // console.log(`[InMemoryPersistence] Loaded state detail:`, JSON.stringify(stateRecord, null, 2));
      return { ...stateRecord }; // Return a copy
    }
    console.log(`[InMemoryPersistence] No state found for ${workflowInstanceId}.`);
    return null;
  }
  ...
  async clearWorkflowState(workflowInstanceId) {
    if (this.store.has(workflowInstanceId)) {
      ...
      console.log(`[InMemoryPersistence] State cleared for ${workflowInstanceId}.`);
    } else {
      console.log(`[InMemoryPersistence] No state to clear for ${workflowInstanceId}.`);
    }
  }
}
